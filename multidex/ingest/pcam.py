...
from multidex.plotter.models import PSpec
...
ASDF_STEM_PATTERN = re.compile(
    r'SOL\d{4}_p\d{4}_PMA\d{1,4}(-\w+)?', re.UNICODE
)


THUMB_PATH = MULTIDEX_ROOT / "plotter/application/assets/browse/pcam/"

PSPEC_FIELD_NAMES = list(map(attrgetter("name"), PSpec._meta.fields))
...
def find_ingest_files(path: Path, recursive: bool = False):
    ...
    if recursive:
        all_files = []
        for subpath, _dirs, files in os.walk(path):
            for f in files:
                all_files.append(Path(subpath) / f)
        marslab_files = tuple(filter(looks_like_marslab, all_files))
        context_files = tuple(filter(looks_like_context, all_files))
    elif path.is_dir():
...
PCAM_BOOL_FIELDS = [
    field.name.upper()
    for field in PSpec._meta.fields
...
def process_marslab_row(row, marslab_file, obs_images):
    row = row.dropna()
    relevant_indices = [ix for ix in row.index if ix in PSPEC_FIELD_NAMES]
    for filt in set(PSpec.filters).intersection(row.index):
...
def format_for_multidex(frame):
    ...
    y_to_bool(frame, PCAM_BOOL_FIELDS)
...
def ingest_marslab_file(marslab_file, context_df):
    ...
    if "INSTRUMENT" in frame.columns:
        if frame["INSTRUMENT"].iloc[0] != "PCAM":
            print("skipping non-PCAM file: " + marslab_file)
            return False, "does not appear to be a PCAM file", context_df
...
def perform_ingest(
    ...
    *,
    ...
    """
    ingests pcam -marslab.csv files and context image thumbnails generated
