...
def filter_df_from_queryset(queryset, **scale_kwargs):
    ...
    for spectrum in queryset:
        filter_value_list.append({
            filt: value['mean'] for filt, value in
            spectrum.filter_values(**scale_kwargs).items()
        })
...
def intervening(filter_df, spec_model, wave_1, wave_2):
    ...
    """
    waves = [
        spec_model().all_filter_waves()[filt]
        for filt in filter_df.columns
    ]
    intervening_df = filter_df.copy()
    intervening_df.columns = waves
    return intervening_df[[
            column for column in intervening_df.columns
...
def band(filter_df, spec_model, wave_1, wave_2):
    ...
    band_df = filter_df.copy()
    band_df.columns = waves
    return band_df[[
            column for column in band_df.columns
            if max(wave_1, wave_2) >= column >= min(wave_1, wave_2)
    ]]
...
def ref_ratio(filter_df, spec_model, filt_1, filt_2):
    """
